...
object Chapter3L {
  ...
  // 3.24 worst case O(n*m); can be (way) better
  ...
  def hasSubsequence[A](sup:C3List[A], sub:C3List[A]): Boolean = {
    ...
    def immediateSubsequence(isup:C3List[A], isub:C3List[A]): Boolean = isub match {
      ...
      case C3Cons(s,ss) => isup match {
        case C3Nil => false // match, but isub is longer than isup -- break
        case C3Cons(x,xs) => s == x /* true if this is a match */ &&
          /* so, shift right and try next */ immediateSubsequence(xs,ss) // or break and return false
      }
    }
    ...
    val (stub,predicate) = sub match {
      ...
      case C3Cons(s,ss) =>
        ...
        start match {
          case C3Nil => (C3Nil, false) // sub begins with something not present in sup
          case C3Cons(_,xs) => (xs, immediateSubsequence(xs,ss)) /* test the rest */
        }
    }

    predicate match {
      case true => true
      case false => stub match {
        case C3Nil => false /* break: we're out of options (end of sup and no hope of match) */
        case _ =>
          if (lenl(stub) < lenl(sub)) false /* break; we're out of options (remaining sup is shorter than sub) */
          else hasSubsequence (stub, sub) /* shift right and try again using stub (the as-yet-untested part of sup) */
      }
    }
  }
